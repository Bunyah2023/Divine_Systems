...
public class PreguntaDao {
    ...
    static {
        try {
            ...
            config.setMaximumPoolSize(50); // Ajusta este valor según lo necesario
    ...
    // Método para obtener preguntas de forma aleatoria con opciones también aleatorias
    public List<Pregunta> obtenerPreguntasAleatoriasPorIdExamen(int idExamen) {
        List<Pregunta> preguntas = new ArrayList<>();
        String sql = "CALL ObtenerPreguntasAleatorias(?)";
        try (Connection con = dataSource.getConnection();
            ...
            System.err.println("Error al obtener preguntas aleatorias por examen ID: " + e.getMessage());
            e.printStackTrace();
        }
        return preguntas;
    }
    ...
    // Método para obtener preguntas por examen
    public List<Pregunta> obtenerPreguntasPorExamen(int examenId) {
        List<Pregunta> preguntas = new ArrayList<>();
        String sql = "SELECT * FROM preguntas WHERE examen_id = ?";
        try (Connection con = dataSource.getConnection();
             PreparedStatement ps = con.prepareStatement(sql)) {
