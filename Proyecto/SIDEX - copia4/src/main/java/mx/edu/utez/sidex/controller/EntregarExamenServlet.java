...
public class EntregarExamenServlet extends HttpServlet {
    ...
    private Resultado calcularResultado(int examenId, Map<Integer, Integer> respuestas, int estudianteId, int claseId, ExamenDao examenDao, User user) {
        ...
        for (Map.Entry<Integer, Integer> entry : respuestas.entrySet()) {
            ...
            if (respuestasCorrectas.get(preguntaId) != null && respuestasCorrectas.get(preguntaId).equals(respuestaSeleccionada)) {
                aciertos++;
            } else {
    ...
    private String obtenerCalificacionCategoria(double calificacion, Map<String, Double> rangosCalificacion) {
        Double minAU = rangosCalificacion.getOrDefault("minAU", 0.0);
        Double maxAU = rangosCalificacion.getOrDefault("maxAU", 10.0);
        Double minDE = rangosCalificacion.getOrDefault("minDE", 0.0);
        Double maxDE = rangosCalificacion.getOrDefault("maxDE", 10.0);
        Double minSA = rangosCalificacion.getOrDefault("minSA", 0.0);
        Double maxSA = rangosCalificacion.getOrDefault("maxSA", 10.0);

        // Imprimir para depuración
        System.out.println("Rangos de Calificación: " + rangosCalificacion);

        // Verificar si los rangos son válidos
        if (minAU > maxAU || minDE > maxDE || minSA > maxSA) {
            System.out.println("Error: Los rangos de calificación no son válidos.");
